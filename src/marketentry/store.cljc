(ns marketentry.store
  "SSoT for the Republic of El Salvador (SLV) market-entry compliance
...
(defn demo-data
  ...
  `:foreign-entity?` / `:home-country-constituted?` /
  `:local-registro-comercio-inscrito?` / `:claimed-eligible?` are ground
  truth for the `foreign-entity-eligibility-mismatch` flagship check
  (Ley de Compras Públicas Art. 24 literal f, Código de Comercio Arts.
  358-360)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Comercializadora San Salvador, S.A. de C.V." :portal "DINAC / COMPRASAL (www.comprasal.gob.sv)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :foreign-entity? false :home-country-constituted? nil :local-registro-comercio-inscrito? nil
             :claimed-eligible? true
             :drafted? false :submitted? false
             :jurisdiction "SLV" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "DINAC / COMPRASAL (www.comprasal.gob.sv)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :foreign-entity? false :home-country-constituted? nil :local-registro-comercio-inscrito? nil
             :claimed-eligible? true
    ...
    "eng-3" {:id "eng-3" :operator "Distribuidora Volcán Verde, S.A. de C.V." :portal "DINAC / COMPRASAL (www.comprasal.gob.sv)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "SLV" :status :intake}
    "eng-4" {:id "eng-4" :operator "Pacific Rim Trading Corp. (sucursal El Salvador)" :portal "DINAC / COMPRASAL (www.comprasal.gob.sv)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               foreign-entity? home-country-constituted? local-registro-comercio-inscrito? claimed-eligible?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? foreign-entity?)                    (assoc :engagement/foreign-entity? foreign-entity?)
    (some? home-country-constituted?)          (assoc :engagement/home-country-constituted? home-country-constituted?)
    (some? local-registro-comercio-inscrito?)  (assoc :engagement/local-registro-comercio-inscrito? local-registro-comercio-inscrito?)
    (some? claimed-eligible?)                  (assoc :engagement/claimed-eligible? claimed-eligible?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/foreign-entity? :engagement/home-country-constituted? :engagement/local-registro-comercio-inscrito?
   :engagement/claimed-eligible?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :foreign-entity? (boolean (:engagement/foreign-entity? m))
     :home-country-constituted? (boolean (:engagement/home-country-constituted? m))
     :local-registro-comercio-inscrito? (boolean (:engagement/local-registro-comercio-inscrito? m))
     :claimed-eligible? (boolean (:engagement/claimed-eligible? m))
