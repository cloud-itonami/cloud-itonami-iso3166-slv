(ns marketentry.marketentryllm
  ...
  the Republic of El Salvador (SLV) public-sector market-entry
  compliance actor.
  ...
  cited), never a committed record or a real DINAC/COMPRASAL filing /
  Registro de Comercio inscription. Every output is censored downstream
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world DINAC/COMPRASAL filing /
  Registro de Comercio inscription request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けDINAC/COMPRASAL提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "foreign-entity?=" (:foreign-entity? e)
                        " home-country-constituted?=" (:home-country-constituted? e)
                        " local-registro-comercio-inscrito?=" (:local-registro-comercio-inscrito? e)
                        " claimed-eligible?=" (:claimed-eligible? e)
     ...
     :confidence (if (and e (or (not (:foreign-entity? e))
                                (:local-registro-comercio-inscrito? e)))
                   0.9 0.3)}))
