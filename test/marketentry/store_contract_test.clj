...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X SA de CV" :jurisdiction "SLV"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :foreign-entity? true :home-country-constituted? true
                                   :local-registro-comercio-inscrito? true :claimed-eligible? true
                                   :drafted? false :submitted? false :status :intake}})
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "SLV" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:foreign-entity? (:engagement m)) (:foreign-entity? (:engagement d))))
    (is (= (:local-registro-comercio-inscrito? (:engagement m)) (:local-registro-comercio-inscrito? (:engagement d))))
    (is (= (:claimed-eligible? (:engagement m)) (:claimed-eligible? (:engagement d))))
