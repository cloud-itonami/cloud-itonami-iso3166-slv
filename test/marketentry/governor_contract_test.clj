  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Comercializadora San Salvador, S.A. de C.V."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Comercializadora San Salvador, S.A. de C.V." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-entity-eligibility-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-eligible? does not match the independently recomputed Ley de Compras Públicas Art.24.f / Código de Comercio Art.358-360 eligibility test -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-entity-eligibility-mismatch} (-> (store/ledger db) last :basis)))
